...
- **Frames Per Second (FPS)**: Games that use requestAnimationFrame may not work well in some situations. For example, in the Ski game, on a phone set at 90hz, the game will change speed after a click event. This is because the browser tries to run at 60hz to save battery but will increase hz if it detects more load. A workaround is to set your phone to 60hz. You can also watch the FPS change at [test](https://www.testufo.com) when triggering an event (for example clicking).

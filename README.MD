...
If this editor doesn't support your needs, try Neovim with Termux and Apache. You can find my configuration for those tools [here](https://github.com/bacionejs/termux).

## Issues
- **Cursor Misalignment**: Swiping up hard at the end of the file in the editor window can cause the cursor to become misaligned with the text. A workaround is to back off a little bit from the end.
...
- **Save Mechanism:** The save function in the editor is limited by HTML constraints. As a workaround, a **new** timestamped file is generated each time you save. For simplicity, the games and editor are saved together as one file. If you're saving to your Downloads folder, it's important to keep it  free of files from other purposes, as this can make it difficult to quickly reopen your work. Also, periodically delete older editor HTML files, keeping only the most recent version.
...
- **Rename**: Highlights matches as you type, but the Find button remains disabled until a whole-word match is detected.
